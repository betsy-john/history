...
var devTasks = ['env-dev',   'libs', 'bootstrap-fonts', 'font-awesome-fonts', 'images', 'sass', 'index', 'scripts', 'simple-server', 'watch'];
var prodTasks = ['env-prod', 'libs', 'bootstrap-fonts', 'font-awesome-fonts', 'images', 'sass', 'index', 'minify-scripts'];
...
gulp.task('simple-server', function () {
   return connect.server(config.serve);
});

gulp.task('watch', function () {

    gulp.watch(config.path.sass.all, ['sass', 'css-reload']);
    gulp.watch(config.path.views.src, ['index', 'html-reload', 'index-reload']);
...
gulp.task('default', ['simple-server'])
